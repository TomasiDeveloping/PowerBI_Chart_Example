...
export class Visual implements IVisual {
  ...
  public update(options: VisualUpdateOptions) {
    const widht = options.viewport.width;
    ...
    if (
      ...
      for (let i = 0; i < category.values.length; i++) {
        extractedData.push({
          ...
          color: this.host.colorPalette.getColor(category.values[i] as string)
            .value,
        });
      }
    }
    ...
    this.svg.attr("width", widht).attr("height", height);

    const bars = this.barContainer.selectAll(".bar").data(extractedData);
